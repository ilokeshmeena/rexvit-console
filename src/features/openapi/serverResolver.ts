 ...
 */
export function getDefaultServerVariables(
  server: OpenApiServer | undefined,
): Record<string, string> {
  if (!server?.variables) return {};

  return Object.entries(server.variables).reduce<Record<string, string>>(
    (acc, [name, varDef]) => {
...
export function getServerVariableOptions(
  ...
  return variable.enum && variable.enum.length > 0
    ? variable.enum
    : [variable.default];
}
