...
describe("createServiceFromOpenApi", () => {
  it("extracts endpoints from OpenAPI paths", () => {
    ...
    expect(service.versions[0]).toMatchObject({
      label: "billing",
      specPath: "/specs/billing.yaml",
    });
    expect(service.endpoints).toHaveLength(2);
    expect(service.endpoints[0]).toMatchObject({
      method: "GET",
      path: "/invoices",
      operationId: "listInvoices",
      version: "billing",
  ...
  it("groups versions by service folder", () => {
    ...
    expect(services[0].versions.map((version) => version.label)).toEqual([
      "v1",
      "v2",
    ]);
  ...
  it("extracts parameter metadata and resolves request body schemas", () => {
    ...
    expect(endpoint.requestBodyRequiredProperties).toEqual({
