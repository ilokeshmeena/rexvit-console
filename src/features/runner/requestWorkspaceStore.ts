...
type RequestWorkspaceState = PersistedWorkspace & {
  hydrated: boolean;
  hydrate: () => void;
  openEndpointRequest: (input: { endpoint: ApiEndpoint; version: ApiVersion | null; request: ApiRequest }) => RequestInstance;
...
export const useRequestWorkspaceStore = create<RequestWorkspaceState>((set, get) => ({
  requests: [],
  openTabs: [],
  ...
  openAdHocRequest: (request) => {
    const instance = createAdHocRequest(get().requests.filter((item) => !item.endpointId).length + 1, request);
    set((state) => ({
      requests: [...state.requests, instance],
      openTabs: [...state.openTabs, toTab(instance)],
      activeRequestId: instance.requestId
  ...
  updateRequest: (requestId, patch) => {
    ...
    set((state) => ({
      requests: state.requests.map((request) =>
        request.requestId === requestId ? { ...request, ...patch, dirty: patch.dirty ?? true, updatedAt: now } : request
      ),
      openTabs: state.openTabs.map((tab) =>
        tab.requestId === requestId ? { ...tab, title: patch.name ?? tab.title } : tab
      )
    }));
    persist(get());
  },
  ...
  closeTab: (requestId) => {
    set((state) => {
      const openTabs = state.openTabs.filter((tab) => tab.requestId !== requestId);
      const activeRequestId = state.activeRequestId === requestId ? openTabs.at(-1)?.requestId ?? null : state.activeRequestId;
  ...
  restoreSnapshot: (request) => {
    const restored = cloneRequestInstance({ ...request, name: `${request.name} Restored` });
    set((state) => ({
  ...
  activeRequest: () => get().requests.find((request) => request.requestId === get().activeRequestId) ?? null
}));

function toTab(request: RequestInstance): OpenRequestTab {
  return { requestId: request.requestId, tabKey: request.tabKey, title: request.name };
}

function readWorkspace(): PersistedWorkspace {
  const fallback: PersistedWorkspace = { requests: [], openTabs: [], activeRequestId: null };
  try {
