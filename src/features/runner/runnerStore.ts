...
export const useRunnerStore = create<RunnerState>((set, get) => ({
  ...
  updateHeader: (index, header) =>
    set((state) => ({
      request: {
        ...state.request,
        headers: state.request.headers.map((item, itemIndex) =>
          itemIndex === index ? { ...item, ...header } : item,
        ),
      },
    })),
  addHeader: () =>
  ...
  removeHeader: (index) =>
    set((state) => ({
      request: {
        ...state.request,
        headers: state.request.headers.filter(
  ...
  updateQueryParam: (index, queryParam) =>
    set((state) => ({
      request: {
        ...state.request,
        queryParams: state.request.queryParams.map((item, itemIndex) =>
          itemIndex === index ? { ...item, ...queryParam } : item,
        ),
      },
    })),
  addQueryParam: () =>
  ...
  removeQueryParam: (index) =>
    set((state) => ({
      request: {
        ...state.request,
        queryParams: state.request.queryParams.filter(
          (_, itemIndex) => itemIndex !== index,
        ),
      },
    })),
...
function demoResponse(): ApiResponse {
  return {
    ...
    body: JSON.stringify(
      {
        data: [{ id: "usr_01", name: "Ada Lovelace", role: "admin" }],
        nextCursor: null,
      },
      null,
      2,
